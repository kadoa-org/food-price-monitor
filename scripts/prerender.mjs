...
function seo(page, path) {
  ...
  if (page.kind === 'home') {
    ...
    title = `${short} Prices Today: Price Chart and USDA Wholesale History`;
    description = `${short} prices today: ${b ? `${quoteText(b.latest)} ${unit(b.package)} at ${b.market}` : 'latest USDA quote'}. Daily USDA wholesale and shipping point prices from ${day(s.firstDate)} to ${day(s.lastDate)}, ${fmt(s.seriesCount)} products in ${s.markets} markets, with a price chart, year-on-year change and free CSV download.`;
    crumbs.push({ name: 'Commodities', url: `${SITE}/food-prices/commodities` }, { name: s.name, url: canonical });
    ld = [{ '@context': 'https://schema.org', '@type': 'Dataset', name: `${s.name} prices, USDA wholesale and shipping point quotes`, description, url: canonical, keywords: [`${s.name.toLowerCase()} prices`, `${s.name.toLowerCase()} prices today`, `${s.name.toLowerCase()} price chart`, 'wholesale produce prices', 'USDA market news'], temporalCoverage: `${s.firstDate}/${s.lastDate}`, spatialCoverage: 'United States', license: 'https://www.usa.gov/government-works', isBasedOn: 'https://mymarketnews.ams.usda.gov/', creator: { '@type': 'Organization', name: 'Kadoa', url: SITE }, dateModified: updated, distribution: [{ '@type': 'DataDownload', encodingFormat: 'text/csv', contentUrl: `${dataPath.startsWith('http') ? dataPath : SITE + dataPath}/downloads/${s.slug}.csv` }] }];
    ...
    title = 'Grocery Prices This Week: Supermarket Ad Prices for Fresh Produce by US Region';
    description = `Grocery prices this week${page.week ? ` (week ending ${day(page.week)})` : ''}: what US supermarkets advertised for fresh produce by region, from USDA's weekly survey of grocery ads, with last week's and last year's price.`;
